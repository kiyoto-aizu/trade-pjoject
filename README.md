...
`data/` は、処理の入力データ、実行結果、運用時の状態を保存する領域です。Gitでは原則として無視されるため、必要なデータはバックアップしてください。

| パス | 保存内容 | 利用する処理・目的 |
| --- | --- | --- |
| `data/analysis/` | ADX、ATR比率、市場ボラティリティ、VIX、LLMエラー状態 | 分析コマンドの結果確認と、取引前の市場レジーム判定の検証に使います。 |
| `data/backtest/` | 最新結果、日時付き履歴、比較結果 | 売買ルール変更後の成績比較、および週次・月次レポートの集計元になります。 |
| `data/dairy-notes/` | 日付ごとの短い運用・開発メモ。現行プログラムの必須入出力ではありません。 | タスク実行や設定変更の記録を残すために手動で使います。 |
| `data/diary/` | 振り返りと検証記録。現行プログラムの必須入出力ではありません。 | 日報作成時の補足メモとして参照し、作業・検証の経緯を振り返るために使います。 |
| `data/filtering/` | 日付別の抽出銘柄と出来高急騰率など | 当日の取引対象を決定し、時系列バックテストでその日の対象銘柄を再現するために使います。 |
| `data/logs/` | pytestや各バッチ処理のログ | 処理失敗の原因調査と、定期タスクが実行されたかの確認に使います。 |
| `data/minute_bars/` | 日付・銘柄ごとのJSON形式の分足データ | 既存のJSON分足データを保管・参照する互換用フォルダです。新規保存はParquet形式を使います。 |
| `data/minute_bars_parquet/` | 銘柄・日付単位のParquet形式の分足データ | 分足バックテストの価格系列として読み込み、分足バックフィルの出力先として使います。 |
| `data/regulation/` | `historical_regulations.csv` | 過去日スクリーニング時に、その日時点で除外すべき規制銘柄を判定するために使います。 |
| `data/reports/` | 取引日報、週次・月次の集計結果 | LINE通知の記録を残し、週次・月次分析と日記作成の集計元として使います。 |
| `data/screening/` | 日付別のランキング候補 | 翌営業日のフィルタリングの入力と、過去条件を用いた検証の候補集合として使います。 |
| `data/trading/` | `order_history.json` とペーパートレード口座状態 | 重複注文を防ぎ、ペーパートレードの現金・保有銘柄を再起動後に復元するために使います。 |
| `data/universe/` | `listed_securities.csv` | 過去日時点で上場していた銘柄だけを対象に、日足ベースのランキングを再計算するために使います。 |
| `data/filter_decision_events.sqlite3` | 取引・バックテストの判定イベントと結果をSQLiteで記録します。 | エントリー・決済条件の結果を追跡し、日次・週次・月次分析で判定件数などを集計するために使います。 |

`screening/`、`filtering/`、`regulation/`、`universe/`、分足データは、再現可能な検証に必要なら別途バックアップまたは共有してください。`trading/`、`reports/`、`logs/`、日記・メモは個別の運用記録なので、公開リポジトリには含めない運用を推奨します。
...
MAX_ORDER_AMOUNT_PER_TRADE=30000
