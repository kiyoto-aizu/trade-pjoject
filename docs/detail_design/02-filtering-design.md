...
```
売買代金比 = 当日累計売買代金（kabu /board） ÷ 過去N日平均売買代金（Yahoo Finance）

9:30時点の当日累計値と終日平均値は時間軸が一致しないため、絶対倍率による足切りは行わない。平均売買代金を取得できた候補を売買代金比の相対順位で並べ、上位10件を採用する。過去20日の同時刻分足を安定取得できるようになった場合は、同時刻帯比較へ置き換える。
```
...
- `filter_by_min_surge_ratio(scored: list[ScoredCandidate], min_ratio: float) -> list[ScoredCandidate]`
  ...
  - 9:30時点と終日平均の時間軸が異なるため、絶対倍率による下限値は設けない
