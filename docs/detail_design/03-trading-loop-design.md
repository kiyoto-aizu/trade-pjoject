...
| 項目 | 考え方 | 判定方法 |
|---|---|---|
| 1回あたりの買い注文上限額 | **30,000円（運用資金100,000円の30%）**。日本株の売買単位を基準に、買い注文ごとの数量を上限内で動的に決める | `GET /apisoftlimit`で取得した上限値と、`config/config.py`の`MAX_ORDER_AMOUNT_PER_TRADE`の**小さい方**から購入可能な最大単元数を計算。1単元も買えない銘柄だけ見送り、当日の取引全体は停止しない |
