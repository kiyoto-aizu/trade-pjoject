...
- **RSIの計算**: RSI14は本日を除く確定終値から計算する。初期実装は直近14変化の単純平均方式とし、`avg_loss == 0`の場合は100、`avg_gain == 0`の場合は0とする。
- **データ不足時**: 確定終値が15本未満、または価格データに不正値がある場合はシグナルを生成しない。推測値・未確定の当日終値は使用しない。
...
- Yahoo Financeの取得範囲は`30d`とし、祝日・休場日を考慮してRSI14に必要な確定終値15本以上を確保する。
- Yahoo FinanceのタイムスタンプをJSTへ変換し、本日の日付と一致するバーを除外する。
- 戻り値は本日を除く確定終値の時系列とし、SMA5・RSI14の計算に必要な本数を保持する（末尾5本だけに切り詰めない）。
...
1. **RSI14の単純平均方式を、十分な履歴取得後にWilder方式へ変更するか**
2. **運用資金額そのもの、および想定損失上限の具体的な割合（何%か）**、1回あたりの発注上限額の具体的な金額 — これらは実際の運用資金が決まった段階で確定する数値パラメータ
3. ②の完了が9:35に間に合わなかった場合の具体的なリトライ・アラート方法（§1のガードはあるが、通知先・再実行手順は未設計）
4. ⑤の都度API取得によるレート制限（`/apisoftlimit`）への抵触有無の実測確認
