...
- **判定方式**: ①②の銘柄選定は変更せず、現在の平均回帰条件にRSIの行き過ぎ判定をANDで追加する。
  - 買い: `current_price <= SMA5 * 0.99` かつ `RSI14 <= 30`
  - 売り: `current_price >= SMA5 * 1.01` かつ `RSI14 >= 70`
...
| 項目 | 考え方 | 判定方法 |
|---|---|---|
| 1回あたりの発注上限額 | **10,000円（運用資金100,000円の10%）**。kabu側の`/apisoftlimit`より厳しい固定額をアプリ側で設定する | `GET /apisoftlimit`で取得した上限値と、`config/config.py`の`MAX_ORDER_AMOUNT_PER_TRADE`の**小さい方**を実効上限として⑤で比較 |
...
| `RSI_BUY_THRESHOLD` | 30 | 売られすぎとみなす上限 |
| `RSI_SELL_THRESHOLD` | 70 | 買われすぎとみなす下限 |
...
- ①②は勢いのある銘柄を選び続け、③だけに行き過ぎ判定ゲートを追加する
- 平均回帰条件とRSI14の行き過ぎ条件はANDで組み合わせる
- RSI期間は14、買い/売り閾値は30/70
