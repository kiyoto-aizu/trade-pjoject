...
class OpenAIDailyAnalyzer:
    ...
    def analyze(self, daily_summary: dict) -> str | None:
        prompt = (
            ...
            "atr_danger_skipsがあれば、見送り後の観測結果から損失回避または利益取り逃しのどちらの可能性が"
            "あるかを評価し、観測終了時刻までの概算であり実取引損益ではないことを明記してください。"
            "atr_stop_exitsがあれば、ATR損切り売却後の観測結果から下落回避または早すぎる決済のどちらの可能性が"
            "あるかを評価し、観測終了時刻までの概算であり実取引損益ではないことを明記してください。"
            "market_regime_danger_skipsとmarket_regime_caution_rsi_filtersがあれば、各見送り後の観測結果から"
            "損失回避または利益取り逃しのどちらの可能性があるかを評価してください。"
            "adx_trend_reliefsがあれば、緩和して取引した後の観測結果から、緩和取引が有利または不利だった可能性を"
            "評価してください。これらも観測終了時刻までの概算であり実取引損益ではないことを明記してください。"
